...
export default function NewBookingPage() {
  ...
  useEffect(() => {
    const loadFacilities = async () => {
      try {
        ...
        setAlertMessage("Failed to load facilities. Please refresh.")
      } finally {
  ...
  const validate = (state: FormState): FormErrors => {
    ...
    if (!state.facilityId) nextErrors.facilityId = "Facility is required"
    ...
    if (!state.startTime) nextErrors.startTime = "Start time is required"
    if (!state.endTime) nextErrors.endTime = "End time is required"
  ...
  const updateField = (field: keyof FormState, value: string) => {
    ...
    setForm(nextForm)
    if (submitAttempted) setErrors(validate(nextForm))
  }
  ...
  return (
    <div className="min-h-svh bg-[#030303] text-white px-6 py-10 md:px-10">
      <div className="mx-auto max-w-3xl space-y-6">
        <Link to="/bookings/my" className="inline-flex items-center text-sm text-white/60 hover:text-white transition">
          {"< "}Back to My Bookings
        </Link>
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.02] p-6 md:p-8">
          ...
          <form className="mt-8 space-y-6" onSubmit={handleSubmit} noValidate>
            <div>
              <label htmlFor="facilityId" className="block text-sm font-bold text-white/80 mb-2">Facility</label>
              <select
                id="facilityId"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input id="date" type="date" min={minDate} value={form.date} onChange={(e) => updateField("date", e.target.value)} className="w-full rounded-2xl border border-white/10 bg-white/[0.02] px-4 py-3 text-white outline-none focus:border-indigo-400" />
              ...
              <div>
                ...
                <input id="startTime" type="time" value={form.startTime} onChange={(e) => updateField("startTime", e.target.value)} className="w-full rounded-2xl border border-white/10 bg-white/[0.02] px-4 py-3 text-white outline-none focus:border-indigo-400" />
              ...
              <div>
                ...
                <input id="endTime" type="time" value={form.endTime} onChange={(e) => updateField("endTime", e.target.value)} className="w-full rounded-2xl border border-white/10 bg-white/[0.02] px-4 py-3 text-white outline-none focus:border-indigo-400" />
            ...
            <div>
              ...
              <textarea id="purpose" rows={3} maxLength={500} value={form.purpose} onChange={(e) => updateField("purpose", e.target.value)} className="w-full rounded-2xl border border-white/10 bg-white/[0.02] px-4 py-3 text-white outline-none focus:border-indigo-400 resize-y" />
            ...
            <div>
              ...
              <input id="attendees" type="number" min={1} value={form.attendees} onChange={(e) => updateField("attendees", e.target.value)} placeholder="Optional" className="w-full rounded-2xl border border-white/10 bg-white/[0.02] px-4 py-3 text-white outline-none focus:border-indigo-400" />
              {errors.attendees && <p className="mt-2 text-sm text-red-400">{errors.attendees}</p>}
            </div>

            <button type="submit" disabled={isSubmitting} className="w-full md:w-auto md:min-w-64 inline-flex items-center justify-center gap-2 rounded-2xl border border-indigo-500/30 bg-indigo-500/20 px-6 py-3 text-sm font-bold uppercase tracking-widest text-indigo-200 hover:bg-indigo-500/30 disabled:opacity-60 disabled:cursor-not-allowed transition">
